...
<section class="overflow-hidden rounded-2xl border border-line bg-paper transition-colors {mode === 'packing' && percent === 100 ? 'border-[#c8d4b7]' : ''}">
	<div class="flex items-center pr-3 sm:pr-4">
		<button class="flex min-h-19 min-w-0 flex-1 items-center gap-3 px-4 py-4 text-left sm:gap-3.5 sm:px-5" aria-expanded={isOpen} aria-controls={contentId} onclick={() => expanded = !expanded}>
			...
			{#if mode === 'packing' && percent === 100}<span in:scale={{ start: 0.6, duration: prefersReducedMotion.current ? 0 : 220, easing: backOut }} class="grid size-6 shrink-0 place-items-center rounded-full bg-forest text-lime"><Icon name="check" size={14} /></span>{:else}<span class="text-[11px] tabular-nums text-muted">{mode === 'packing' ? `${percent}%` : String(items.length).padStart(2, '0')}</span>{/if}
	...
	{#if isOpen}
		<div id={contentId} transition:slide={{ duration: prefersReducedMotion.current ? 0 : 240, easing: cubicOut }} inert={!isOpen} class="border-t border-line">
			<ul class="px-4 sm:px-5">
				{#each visibleItems as item (item.id)}
					<li transition:slide={{ duration: prefersReducedMotion.current ? 0 : 180, easing: cubicOut }} class="group flex min-h-12 items-center gap-2 border-b border-line/70 py-1 last:border-b-0 sm:gap-3">
						{#if mode === 'packing'}
							<label class="flex min-h-11 min-w-0 flex-1 cursor-pointer items-center gap-3 py-1">
								...
								<span class="grid size-5 shrink-0 place-items-center rounded-md border transition-all peer-focus-visible:ring-2 peer-focus-visible:ring-forest peer-focus-visible:ring-offset-2 {item.packed ? 'border-forest bg-forest text-white' : 'border-[#cccec1] bg-white'}">{#if item.packed}<span class="grid place-items-center" in:scale={{ start: 0.5, duration: prefersReducedMotion.current ? 0 : 200, easing: backOut }}><Icon name="check" size={13} /></span>{/if}</span>
								<span class="min-w-0 text-[13px] leading-relaxed transition-colors duration-200 {item.packed ? 'text-muted line-through decoration-[#afb5a1]' : ''}">{item.name}{#if item.quantity > 1}<span class="ml-2 inline-block rounded-md bg-canvas px-1.5 py-0.5 text-[10px] text-muted no-underline">×{item.quantity}</span>{/if}{#if item.note && !item.packed}<span class="mt-0.5 block text-[10px] leading-relaxed text-muted">{item.note}</span>{/if}</span>
							</label>
						{:else}
